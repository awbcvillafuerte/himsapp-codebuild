...
import {
  generateRequestArrayIcd10,
  generateRequestArrayCpt,
  listToMatrix,
} from '../../utils';
...
const LoginPage = (props: any) => {
  ...
  const redirect = async () => {
    ...
    try {
      ...
      redirecting = true;
      let modules: any = [];
      let token = await loginStorageService.getSingleEntryByKeyReturnValue(
        'user_data',
        'access_token',
      ); // GET ACCEESS TOKEN
      let urls = await fetch(
        `${process.env.REACT_APP_HIMS_API_CLIENT_URL}modules`,
        {
          method: 'GET',
          headers: {
      ...
      let foundIndex = modules.findIndex((el: any) =>
        Boolean(
          el.name && firstActiveRole && firstActiveRole.module === el.name,
        ),
      );

      console.log(
        `FOUND INDEX ${foundIndex} from array ${JSON.stringify(
          modules,
          null,
          2,
        )}`,
      ...
      if (foundIndex >= 0) {
        ...
        if (
          module.name.toLowerCase() === 'customer care' &&
          groupName === 'CSS GROUP'
        )
          window.location.replace(CSSModuleUrl);
        else window.location.replace(module.dashboard_url);
      ...
      // NOTE (TJ): Fallback redirect statements incase module fetch to failed
      console.log(err);
    ...
    if (count) {
      ...
      for (let i = 0; i < chunkRequest.length; i++) {
        ...
        const promiseCall = () => {
          startTime = moment();
          return Promise.all(
            chunkRequest[i].map((promise: any) =>
      ...
      let saveEntry = await loginStorageService
        .saveEntry(icd10_collection, 'icd10_list')
        .catch(err => console.log(err));

      if (saveEntry) {
        icd10FetchDone = true;
        let saveUpdatedIcd10Config = await loginStorageService
          .saveEntry(icd10ToSave, 'icd10')
          .catch(err => console.log(err));
  ...
  const fetchIcd10Update = async () => {
    let query = await loginStorageService.getSingleEntryByKeyReturnValue(
      'icd10',
      'date_updated',
    );
  ...
  const fetchCpt = async (data: any, defaultSkip?: number) => {
    ...
    if (count) {
      ...
      let saveEntry = await loginStorageService
        .saveEntry(cpt_collection, 'cpt_list')
        .catch(err => console.log(err));

      if (saveEntry) {
        cptFetchDone = true;
        let saveUpdatedCptConfig = await loginStorageService
          .saveEntry(cptToSave, 'cpt')
          .catch(err => console.log(err));
  ...
  const saveToIndexedDB = async (data: any) => {
    ...
    loginStorageService
      .getSingleEntryByKeyReturnValue('icd10', 'juday')
      .then((juday: any) => {
        ...
        if (newJuday && moment(newJuday).isAfter(existingJuday)) {
          ...
          loginStorageService
            .getSingleEntryByKeyReturnValue('icd10', 'date_updated')
            .then(du => {
              if (!moment(data.icd10.date_updated).isAfter(du.result)) {
                loginStorageService
                  .validateStoreCount('himsDb', 'icd10_list')
                  .then((res: number) => {
                    // console.log(res);
                    if (res === 0) {
                      setOpen(true);
                      ...
                      icd10FetchDone = true;
                      loginStorageService
                        .validateStoreCount('himsDb', 'cpt_list')
                        .then((res: number) => {
                          if (res > 0) {
                            redirect();
                          }
                ...
                fetchIcd10Update();
                loginStorageService
                  .updateEntry('icd10', 'date_updated', data.icd10.date_updated)
                  .then(r => {})
                  .catch(err => console.log(err));
              }
            })
        ...
        loginStorageService
          ...
          .catch(() => {
            ...
            setModalProps({
              ...
              title: 'Error',
              message: (
                <span>Something went wrong in indexedDB. Please try again</span>
              ),
              buttonText: 'Okay',
            });
          });
    ...
    loginStorageService
      .getSingleEntryByKeyReturnValue('cpt', 'juday')
      .then((juday: any) => {
        ...
        if (newJuday && moment(newJuday).isAfter(existingJuday)) {
          ...
          loginStorageService
            .getSingleEntryByKeyReturnValue('cpt', 'date_updated')
            .then(du => {
              if (!moment(data.cpt.date_updated).isAfter(du.result)) {
                ...
                fetchCptUpdate();
                loginStorageService
                  .updateEntry('cpt', 'date_updated', data.cpt.date_updated)
                  .then(r => {
                    // console.log(r);
                  })
                  .catch(err => console.log(err));
  ...
  const login = async () => {
    ...
    localStorage.setItem(
      'CLIENT_URL',
      process.env.REACT_APP_HIMS_API_CLIENT_URL!,
    );
    localStorage.setItem(
      'PARTNER_URL',
      process.env.REACT_APP_HIMS_API_PARTNER_URL!,
    );
    localStorage.setItem(
      'CLAIMS_URL',
      process.env.REACT_APP_HIMS_API_CLAIMS_URL!,
    );
    ...
    localStorage.setItem(
      'FRANCHISING_URL',
      process.env.REACT_APP_HIMS_API_FRANCHISING!,
    );
    ...
    localStorage.setItem(
      'PMAKER_BASE_URL',
      process.env.REACT_APP_PMAKER_BASE_URL!,
    );
    ...
    await fetch(url, options)
      ...
      .then(async (data: any) => {
        if (!data.error) {
          ...
          if (
            Array.isArray(login_roles) &&
            login_roles.length &&
            main_role_id
          ) {
            const activeMainRole: any = login_roles.find(
              rl => rl.role_id === main_role_id && rl.status === 'active',
            );
            if (activeMainRole) {
          ...
          loginStorageService
            .saveEntry(userDataToSave, 'user_data')
            .then(res => {
              // console.log(res);
            })
            .catch(err => console.log(err));
          ...
          await fetch(
            `${process.env.REACT_APP_HIMS_API_CLIENT_URL}system-settings/config`,
            {
              method: 'GET',
              headers: {
                'Content-type': 'application/json',
          ...
          if (data.error.message.includes('UM06')) {
            ...
            setModalProps({
              ...
              message: (
                <span>
                  Please contact your <strong>Department Head</strong> to unlock
                  your account.
                </span>
              ),
              buttonText: 'Okay',
            ...
            setModalProps({
              ...
              message: (
                <span>
                  Please contact your <strong>Department Head</strong> to
            ...
            setModalProps({
              ...
              message: (
                <span>
                  ...
                  <strong>
                    {data.error.message.replace(/Code UM61/, '').trim()}
                  </strong>
                  .
                </span>
              ),
  ...
  const onForgotPasswordSubmit = async (username: string) => {
    ...
    let requestSetup = await fetch(
      `${process.env.REACT_APP_HIMS_API_CLIENT_URL}password/forgot/${username}`,
      request,
    ).catch(err => {
    ...
    if (requestSetup) {
      ...
      if (respjson && !respjson.error) {
        setModalProps({
          ...
          message:
            'A reset password email will be sent to the email address of the account.',
          buttonText: 'Okay',
        });
  ...
  const onSetupPasswordSubmit = async (value: any) => {
    ...
    let requestSetup = await fetch(
      `${process.env.REACT_APP_HIMS_API_CLIENT_URL}user/password`,
      request,
    ).catch(err => {
      // BEGIN HANDLING OF FAILED HTTP REQUEST
      setModalProps({
      ...
      if (respjson && !respjson.error) {
        ...
        if (respjson.error.message.includes('UM90')) {
          ...
          setModalProps({
            ...
            message: (
              <span style={{ display: 'inlne-block', width: 350 }}>
                The last <strong>{attempts[0]}</strong> password/s may not be
                reused as new password.
              </span>
            ),
            buttonText: 'Okay',
          });
    ...
    <form onSubmit={onLogin}>
      ...
      <Grid container className="login-main">
            ...
            {isError ? (
              <div className={classes.errorMessageContainer}>
                ...
                <div className={classes.errorString}>
                  <span className={classes.errText}>{errorMessage}</span>
                </div>
              </div>
            ) : (
              ''
        ...
        <MessageDialog
          ...
          buttonText={
            modalProps.buttonText !== '' ? modalProps.buttonText : 'Okay'
          }
        />
        ...
        <SecondInstanceError
          open={secondInstanceModal}
          onClose={closeSecondInstanceModal}
        />
      </Grid>
      <Backdrop className={classes.backdrop} open={open} onClick={handleClose}>
        <Typography>
          {Math.round(
            ((fetchedCpt + fetchedIcd10) / (totalCpt + totalIcd10)) * 100,
          ) === 100
            ? 'Redirecting...'
            : initializingStatus}
        </Typography>
          ...
          >
            ...
            >
              {`${
                isNaN(
                  Math.round(
                    ((fetchedCpt + fetchedIcd10) / (totalCpt + totalIcd10)) *
                      100,
                  ),
                )
                  ? 0
