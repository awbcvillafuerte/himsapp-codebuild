...
const ProfilePage = (props: any): JSX.Element => {
  ...
  React.useEffect(() => {
    StorageService('user_data', 'user_id')
      .then((data: any) => {
    ...
    let urlApp = localStorage.getItem('CLIENT_URL')
      ? localStorage.getItem('CLIENT_URL')
      : process.env.REACT_APP_HIMS_API_CLIENT_URL;

    setLoading(true);

    StorageService('user_data', 'access_token')
      .then(({ result }: any) => {
        setToken(result);
        fetch(`${urlApp}profilxe`, {
          method: 'GET',
          headers: {
  ...
  const updateProfile = async () => {
    ...
    let update = await fetch(
      `${process.env.REACT_APP_HIMS_API_CLIENT_URL}/users/profile_pic`,
      {
        method: 'PATCH',
        headers: {
    ...
    <>
      ...
      >
        <Fade in={uploadModal}>
          <div className={classes.paper}>
            ...
            <p id="transition-modal-title" style={{ fontSize: '20px' }}>
              <b>Upload Picture</b>
            </p>
            <ImageCropper
      ...
      <Grid container>
              ...
              <Grid item xs={3} className={classes.sideBarContainer}>
                <IconButton edge="end" onClick={() => {}}>
                  <Avatar
                    ...
                    src={
                      userData.profile_pic !== 'DEFAULT_PIC'
                        ? `data:image/jpeg;base64,${userData.profile_pic}`
        ...
        <MsgModal
          ...
          message={
            'Please contact your supervisor or your administrator for assistance.'
          }
        />
      </Grid>
